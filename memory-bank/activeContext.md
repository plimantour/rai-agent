...
- PII remediation fields now default to anonymized category/subcategory suggestions (no extra anonymize button); reviewers can tweak the proposal or revert to the original, and approved false positives remain in the session allowlist so duplicates stay cleared on subsequent scans.
- Added Azure AI Language PII detection to the upload flow with chunked scans, automatic language detection, and configurable domains/categories; the remediation UI now shows a deduplicated list that highlights confidence, category, and occurrence counts while tracking user-approved false positives inside the session so "Proceed" respects their decisions without re-flagging the same terms.
- Centralized HTMX upload ingestion helper now records a `stored_solution_validated` session flag so malware scans and Content Safety checks run only when a genuinely new file lands while reuse paths skip duplicate service calls; upload routes enqueue toast updates for each scan stage, the spinner overlay rotates "virus scan" and "responsible AI" labels during ingestion, and analysis/generation flows trust the stored text without re-triggering scanners. Toast z-index and HTMX swap timing were tweaked so notifications post immediately even while the spinner remains visible until new content renders.
- Added a prompt sanitizer layer after Content Safety: uploaded text is normalized, directive phrases are neutralized or blocked, template markers are escaped, and sanitizer findings are logged. The upload spinner and toasts now surface the sanitizer phase, and only sanitized text is stored for downstream analysis/generation.
