...
- **Medium – Sensitive term leakage during remediation:** Uploaded documents may contain PII or confidential identifiers. Azure AI Language now scans sanitized text in configurable chunks, deduplicates findings, and routes them through a remediation queue where users must anonymize or explicitly approve terms before proceeding; approved terms are stored in a session allowlist so intentional false positives do not resurface.
...
- **PII detection & remediation:** `helpers/pii_sanitizer.scan_text_for_pii` leverages Azure AI Language with chunked scans, automatic language detection, configurable categories/domains, and allowlists; the HTMX UI surfaces deduplicated findings (with occurrence counts) and enforces anonymize-or-approve decisions while persisting per-session approvals back into the detection pipeline.
