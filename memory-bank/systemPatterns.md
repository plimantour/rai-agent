...
- Remediation route stores user-approved PII terms in a per-session allowlist, feeds the additional allowlist into subsequent Azure AI Language scans, deduplicates entities by canonical key so recap cards display unique findings with occurrence counts, and now provides default anonymized replacements per entity while keeping original text visible via placeholders.
