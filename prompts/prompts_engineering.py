...
from prompts.rai_prompts import SYSTEM_PROMPT, TARGET_LANGUAGE_PLACEHOLDER, SOLUTION_DESCRIPTION_PLACEHOLDER
from prompts.rai_prompts import INTENDED_USES_PLACEHOLDER, INTENDED_USES_STAKEHOLDERS, FITNESS_FOR_PURPOSE_PROMPT
from prompts.rai_prompts import STAKEHOLDERS_PROMPT, GOALS_A5_T3_PROMPT, SOLUTION_SCOPE_PROMPT, SOLUTION_INFORMATION_PROMPT
from prompts.rai_prompts import INTENDED_USES_PROMPT, RISK_OF_USE_PROMPT, IMPACT_ON_STAKEHOLDERS_PROMPT, HARMS_ASSESMENT_PROMPT
from prompts.rai_prompts import SOLUTION_INTENDEDUSE_ASSESSMENT_PROMPT, DISCLOSURE_OF_AI_INTERACTION_PROMPT, SOLUTION_DESCRIPTION_ANALYSYS_PROMPT
...
if key_vault_url:
    ...
    if openai.api_type == 'azure':
        ...
        completion_model : str = os.getenv("AZURE_OPENAI_GPT_DEPLOYMENT")
...
def get_azure_openai_completion_nocache(prompt, system_prompt, model=completion_model):
    try:
        if openai.api_type == 'azure':
...
def get_azure_openai_completion(prompt, system_prompt, model=completion_model, json_mode="text", temperature=0.0, language="English", min_sleep=0, max_sleep=0, rebuildCache=False, compress=False, verbose=False):
    ...
    finish_reason = response.choices[0].finish_reason if response and response.choices and len(response.choices) > 0 else "unknown"
    print(colored(f"Completion's finish reason ({finish_reason}).", "yellow"))
        ...
        if answer[0] == '[':    # If the answer is a list, convert it to a dictionary of lists
            ...
            try:
                ...
                try:
                    ...
                    print(e)
                    print(colored(
                        f"Failed to convert the JSON to the expected dictionary.\n{_preview_value(answer)}",
                        "red",
                    ))
                    return {}
            except Exception as e:
                print(e)
                print(colored(
                    "Failed to convert the JSON to a dictionary.\n{}\n------\n{}".format(
                        _preview_value(answer),
...
def process_intended_uses(answer, doc, rai_filepath, nbintendeduses=10, verbose=False):
...
def process_fitness_for_purpose(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_stakeholders(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_goals_a5_t3(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
    ...
    try:
        ...
        if main_key in json_answer:
            ...
            print(colored(f"Failed to process goals A5 and T3 main key.", "red"))
            return {}, [], []
        for intended_use_number in range(1, nbintendeduses+1):
            intended_use_number_str = str(intended_use_number).zfill(2)
            answers_list = None
            ...
            for goal_id in goal_tag_mapping.keys():
                if answers_list:
                    answer = next((answer['detailed_answer'] for answer in answers_list if answer['question'] == goal_id), None)
                else:
                    answer = None
        ...
        print(
            colored(
                f"Failed to process goals A5 and T3.\n{_preview_value(answer)}\n------\n{_preview_value(json_answer)}",
                "red",
            )
        )
        return {}, [], []
...
def process_solution_scope(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_solution_assessment(answer, doc, rai_filepath, intended_uses_list=[], nbintendeduses=10, verbose=False):
...
def process_risk_of_use(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_harms_assessment(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_impact_on_stakeholders(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
    ...
    try:
        ...
        for impact in impact_on_stakeholders:
            intended_use_number_str = str(impact['intendeduse_id']).zfill(2)
...
def process_disclosure_of_ai_interaction(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
# Method to process the solution information section
def process_solution_information(answer, doc, rai_filepath, nbintendeduses=10, intended_uses_list=[], verbose=False):
...
def process_solution_description_analysis(solution_description, language='English', model=completion_model, ui_hook=None, rebuildCache=False, min_sleep=0, max_sleep=0, verbose=False):
    # Update SYSTEM_PROMPT to include the language
    system_prompt = SYSTEM_PROMPT.replace(TARGET_LANGUAGE_PLACEHOLDER, language)

    prompt = SOLUTION_DESCRIPTION_ANALYSYS_PROMPT
...
def update_rai_assessment_template(solution_description, rai_filepath, language='English', model=completion_model, ui_hook=None, rebuildCache=False, update_steps=False, min_sleep=0, max_sleep=0, compress=False, verbose=False):
    ...
    for step_name, prompt, temperature, json_or_text, processor in steps:
        if  prompt == INTENDED_USES_PROMPT or (intended_use_list and prompt != INTENDED_USES_PROMPT):
            ...
            filled_prompt = filled_prompt.replace(INTENDED_USES_STAKEHOLDERS, json.dumps(intendeduses_stakeholders))
            try:
                step_message = f'\nStep {step+1} / {len(steps)}: Generating "{step_name}" with {"Mistral Large" if model == "azureai" else model}'
            ...
            try:
                ...
                if prompt == INTENDED_USES_PROMPT:
                    json_answer, intended_use_list, search_for, replace_by = processor(answer, doc, rai_filepath, verbose=verbose)
                    ...
                    json_answer, intendeduses_stakeholders, search_for, replace_by = processor(answer, doc, rai_filepath, verbose=verbose)
                else:
                    json_answer, search_for, replace_by = processor(answer, doc, rai_filepath, intended_uses_list=intended_use_list, verbose=verbose)
    ...
    for section in sections:
        if section and section.keys():
            ...
            print(colored(f"Section is empty or does not have a main key.\n{_preview_value(section)}", 'red'))
    
    print('\n')
    uiprint(f'Total completion cost: {total_completion_cost:.2f} €', ui_hook=ui_hook, color='yellow')
